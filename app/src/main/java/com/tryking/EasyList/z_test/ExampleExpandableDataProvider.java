...
public class ExampleExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    private final List<Pair<GroupData, List<ChildData>>> mData;
    ...
    public ExampleExpandableDataProvider() {
        final String groupItems = "123456789987654321123456789";
        final String childItems = "abcdefghigklmn";
        mData = new LinkedList<>();

        for (int i = 0; i < groupItems.length(); i++) {
            final long groupId = i;
            final String groupText = Character.toString(groupItems.charAt(i));
            final ConcretGroupData group = new ConcretGroupData(groupId, groupText);
    ...
    public void moveChildItem(int fromGroupPosition, int fromChildPosition, int toGroupPosition, int toChildPosition) {
        ...
        if (toGroupPosition != fromGroupPosition) {
            // assign a new ID
            final long newId = ((ConcretGroupData) toGroup.first).generateNewChildId();
    ...
    public static final class ConcretGroupData extends GroupData {
        private final long mId;
        private final String mText;
        private boolean mPinned;
        private long mNextChildId;

        ConcretGroupData(long id, String text) {
            mId = id;
            mText = text;
            mNextChildId = 0;
        }
